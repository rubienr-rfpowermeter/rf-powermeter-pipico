...
{
  explicit Converter3rdOrder(const CorrectionValues &kValues, float attenuationDb = 0);
  ...
  float toCorrectedDbmW(uint16_t rawValue) const;

  /// Converts the dBmW value to Watts using SI units.
  /// @param[in] correctedDbmW the dBmW value to convert to W
  static constexpr LinearW toLinearW(const float &correctedDbmW);
